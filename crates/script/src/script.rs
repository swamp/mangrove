...
use swamp_script::prelude::{DepLoaderError, ModulePath, Parameter, ResolveError, Type, Variable};
...
use swamp_script_semantic::{ExternalFunctionId, ResolvedParameter, ResolvedProgram, ResolvedType};
...
impl Script {
    ...
    fn boot_call_main(
        ...
        let mut interpreter = Interpreter::<ScriptContext>::new();

        let module_path = ModulePath(vec!["main".to_string()]);
        let mut namespace = ResolvedModuleNamespace::new(module_path);
        /*
        ...
        */
        let whole_program = ResolvedProgram::new();
        ...
        let _script_value_with_signal = interpreter
            .eval_module(&main_module, &mut script_context)
